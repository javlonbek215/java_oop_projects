...
//
//        // getDrivers
        MyArray<Driver> driverCollection = championship.getDrivers();
        boolean test1 = driverCollection.isContains(driver_Ali);
        if (!test1) {
            System.err.println("test1: Haydovchi 'Ali'   topilmadi ERROR");
            System.exit(-1);
        }

        boolean test2 = driverCollection.isContains(driver_Bob);
        if (!test2) {
...
//
//        // defineGrandPri()
        GP gp_1 = championship.defineGrandPri("GP1");
        GP gp_2 = championship.defineGrandPri("GP2");
//
//
//        //getGrandPri()
        GP test5 = championship.getGrandPri("GP1");
        if (test5 == null) {
...
//
        //getGPRanking()
        MyArray<Driver> gp_1_driverList = gp_1.getGPRanking();
        //# test8
        ...
        MyArray<Driver> gp_2_driverList = gp_2.getGPRanking();
        //# test9
...
//
//        //getRaced()
        MyArray<GP> gpCollection_Vali = driver_Vali.getRaced();
        //# test12
        ...
        //# test13
        if (!gpCollection_Vali.isContains(gp_2)) {
            System.out.println("test13: 'Vali' 'GP_2' da qatnashgan ERROR");
            System.exit(-1);
        }

        MyArray<GP> gpCollection_Ali = driver_Ali.getRaced();
        //# test14
        ...
        //# test15
        if (!gpCollection_Ali.isContains(gp_1)) {
...
//        //getChampionshipRanking()
        MyArray<Driver> driverList = championship.getChampionshipRanking();
        //# 18
    ...
    private static String getTimeString(int hours, int min, int sec, int cent) {
